...
- `ros run -l cl-tensor-decomposition-test.asd -e '(asdf:test-system :cl-tensor-decomposition)' -q` runs the Prove suite headlessly.
...
## Testing Guidelines
Tests use Prove with `plan`/`ok` assertions; mirror the naming pattern `tests/<system>.lisp` and define fixtures near the top of each file. New features should add targeted assertions plus coverage around `decomposition`. Run `asdf:test-system :cl-tensor-decomposition` before opening a PR, and document any stochastic behavior (set random seeds when introducing new randomized steps).
